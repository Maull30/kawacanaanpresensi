  ...
  try {
    ...
    if (action === 'get_user_workspaces') {
      ...
      if (!profile && userEmail) {
        ...
        if (profileByEmail) {
          try {
            await db.from('profiles').update({ id: userId, is_active: true }).eq('id', profileByEmail.id);
    ...
    if (action === 'save_student') {
      ...
      // Validasi batas maksimal 50 siswa per kelas (kebijakan sistem paket)
      if (!studentId && classId) {
        const { count: studentCountInClass } = await db
          .from('students')
        ...
        if ((studentCountInClass || 0) >= 50) {
          ...
          return json(res, 400, {
            error: `Kapasitas ${clsName} maksimal 50 siswa. Sistem menolak input karena kuota kelas telah mencapai batas maksimal (50 siswa).`,
          });
        }
      }
      ...
      if (!classId) {
        ...
        if (isPersonalWs) {
          if (callerRole === 'GURU MAPEL') {
            ...
            // Wali Kelas: Kapasitas hanya 1 kelas
            if ((currentClassCount || 0) >= 1) {
              return json(res, 400, {
