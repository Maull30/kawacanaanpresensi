...
export type SystemSubTab = 'siaran' | 'keamanan' | 'gateway' | 'ekspor' | 'akun-diagnostik';
...
const subTabs: { id: SystemSubTab; label: string; icon: any; desc: string }[] = [
  ...
  { id: 'keamanan', label: 'Keamanan & Log Audit', icon: Shield, desc: 'Log forensik, login & aksi kritis' },
  { id: 'gateway', label: 'Gateway Pembayaran', icon: CreditCard, desc: 'Konfigurasi Midtrans Gateway' },
  { id: 'ekspor', label: 'Cadangan & Ekspor', icon: Download, desc: 'Unduh CSV multi-tabel & JSON' },
  { id: 'akun-diagnostik', label: 'Akun & Diagnostik', icon: KeyRound, desc: 'Ganti password & kondisi server' },
];
...
export const SystemSection: React.FC<{
...
}> = ({ call, showToast, activeSubTab = 'siaran', onSubTabChange }) => {
  const { globalAnnouncement, updateGlobalAnnouncement } = useApp();
  const [currentSubTab, setCurrentSubTab] = useState<SystemSubTab>('siaran');
  ...
  // Normalisasi tab aktif eksternal
  useEffect(() => {
    if (activeSubTab) {
      if (['siaran', 'broadcast'].includes(activeSubTab)) {
        setCurrentSubTab('siaran');
      } else if (['keamanan', 'audit', 'security', 'log'].includes(activeSubTab)) {
        setCurrentSubTab('keamanan');
      } else if (['gateway', 'midtrans', 'qris', 'payment_gateway'].includes(activeSubTab)) {
        setCurrentSubTab('gateway');
      } else if (['ekspor', 'data', 'backup', 'cadangan'].includes(activeSubTab)) {
        setCurrentSubTab('ekspor');
      } else if (['akun-diagnostik', 'login', 'akun', 'kondisi', 'health'].includes(activeSubTab)) {
        setCurrentSubTab('akun-diagnostik');
      } else {
        setCurrentSubTab('siaran');
      }
    }
  ...
  const loadGatewayConfig = async () => {
    ...
    try {
      ...
      showToast(e.message || 'Gagal memuat konfigurasi gateway.', 'error');
    } finally {
  ...
  const loadHealth = async () => {
    ...
    try {
      ...
      showToast(e.message || 'Gagal memeriksa kondisi server.', 'error');
    } finally {
  ...
  useEffect(() => {
    if (currentSubTab === 'akun-diagnostik') {
      loadHealth();
    }
  }, [currentSubTab]);

  // Tab 1: Simpan Broadcast
  const handleSaveAnnouncement = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        broadcastActive
          ? 'Pita pengumuman global berhasil diaktifkan ke seluruh instansi sekolah.'
  ...
  // Tab 3: Tes Koneksi Midtrans
  const handleTestMidtrans = async () => {
    setTestingMidtrans(true);
    try {
      const res = await call('test_midtrans');
      if (res.ok) {
        showToast(res.message || 'Koneksi Midtrans Sandbox berhasil & valid.', 'success');
      } else {
  ...
  const handleExportPayments = async () => {
    ...
    try {
      ...
      const headers = ['ID Invoice', 'Sekolah', 'Metode', 'Jumlah (Rp)', 'Status', 'Tanggal', 'Keterangan'];
      const rows = payments.map((p: any) => [
        p.invoiceNo || p.invoice_no || p.id,
        p.schoolName || p.school_name || '-',
        p.paymentMethod || p.payment_method || 'Transfer / QRIS',
        p.totalAmount || p.total_amount || p.amount || 0,
        p.status,
        p.createdAt || p.created_at || '-',
        p.notes || '-',
      ]);
      downloadCSV('data_transaksi_pembayaran', headers, rows);
      showToast('Rekap transaksi keuangan berhasil diekspor ke CSV.', 'success');
    } catch (e: any) {
  ...
  // Tab 5: Ubah Password Super Admin
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ========================================================================= */}
      {/* TAB 1: SIARAN PENGUMUMAN (BROADCAST)                                      */}
      {/* ========================================================================= */}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-100">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 text-xs font-bold mb-2">
                <Megaphone size={13} />
                <span>Pita Broadcast Seluruh Sekolah</span>
              </div>
              <h3 className="text-base sm:text-lg font-black text-slate-900">
                Pita Pengumuman Global Multi-Tenant
              </h3>
              <p className="text-xs text-slate-500 mt-1 max-w-2xl leading-relaxed">
                Pesan ini langsung disiarkan di bagian paling atas dashboard seluruh sekolah, kepala sekolah, guru, dan admin.
              </p>
            </div>

            <label className="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer select-none self-start sm:self-auto">
              <input
                type="checkbox"
              ...
              />
              <span>Aktifkan ke Seluruh Sekolah</span>
            </label>
          </div>
          ...
          <form onSubmit={handleSaveAnnouncement} className="space-y-5">
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-slate-500">
                Pratinjau Langsung Tampilan Dashboard Sekolah:
              </label>
              <div
              ...
              >
                ...
                <span className="truncate">
                  {broadcastMsg.trim() || 'Ketik teks pesan untuk melihat pratinjau pita pengumuman...'}
                </span>
              </div>
            </div>
      ...
      {/* ========================================================================= */}
      {/* TAB 2: KEAMANAN & LOG AUDIT (1 TABEL AUDIT TERPADU DENGAN FILTER)         */}
      {/* ========================================================================= */}
      {currentSubTab === 'keamanan' && (
        <SecuritySection
          call={call}
          showToast={showToast}
          activeFilter="all"
      ...
      {/* ========================================================================= */}
      {/* TAB 3: GATEWAY PEMBAYARAN (MIDTRANS & QRIS)                               */}
      {/* ========================================================================= */}
      {currentSubTab === 'gateway' && (
        <div className="space-y-6">
          {loadingGateway ? (
            <div className="bg-white rounded-3xl p-16 flex flex-col items-center justify-center text-slate-400">
              <RefreshCw size={24} className="animate-spin text-indigo-600 mb-2" />
              <span className="text-xs font-semibold">Memuat konfigurasi gateway pembayaran...</span>
            </div>
          ) : (
              ...
              <div className="bg-white p-6 sm:p-7 rounded-3xl border border-slate-200/80 shadow-xs space-y-5">
                <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h3 className="text-sm font-black text-slate-900">Midtrans Payment Gateway</h3>
                      <p className="text-[11px] text-slate-500">Gateway resmi untuk QRIS, Virtual Account & E-Wallet</p>
                    </div>
                  </div>
                ...
                <form onSubmit={handleSaveMidtrans} className="space-y-4 text-xs">
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">Status Gateway</label>
                    <label className="inline-flex items-center gap-2 cursor-pointer select-none">
                      <input
                        ...
                        className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                      />
                      <span className="font-semibold text-slate-800">Aktifkan pembayaran otomatis Midtrans</span>
                    </label>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2 rounded-xl border border-slate-200 font-mono text-xs focus:outline-indigo-600"
                    />
                  </div>

                  <div>
                    <label className="block font-bold text-slate-700 mb-1">Client Key (Publik)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="font-bold text-slate-700">Server Key (Rahasia)</label>
                      {midtransConfig.is_server_key_configured && (
                    ...
                    <input
                      ...
                      placeholder={midtransConfig.is_server_key_configured ? '•••••••••••••••• (Isi untuk mengganti)' : 'SB-Mid-server-...'}
                      className="w-full px-3.5 py-2 rounded-xl border border-slate-200 font-mono text-xs focus:outline-indigo-600"
                    />
                    <p className="text-[10px] text-slate-400 mt-1">
                      Server key hanya disimpan secara aman di backend dan tidak pernah terekspos ke browser client.
                    </p>
                  </div>
                  ...
                  <div className="pt-2 flex items-center justify-between gap-2">
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs transition cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
                    >
                      {testingMidtrans ? <RefreshCw size={13} className="animate-spin" /> : <ExternalLink size={13} />}
                      <span>Uji Koneksi Midtrans</span>
                    </button>

                    <button
                      type="submit"
                      disabled={savingMidtrans}
                      className="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-xs transition cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
                    >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-2">
            ...
            <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col justify-between space-y-4">
              <div className="space-y-2">
                ...
                <div className="text-sm font-bold text-slate-900">Rekap Transaksi Finansial</div>
                <p className="text-xs text-slate-500 leading-relaxed">
      ...
      {/* ========================================================================= */}
      {/* TAB 5: AKUN SUPER ADMIN & DIAGNOSTIK SERVER                               */}
      {/* ========================================================================= */}
      {currentSubTab === 'akun-diagnostik' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Kolom Kiri: Indikator Kesehatan Sistem & Diagnostik (7/12) */}
          <div className="lg:col-span-7 bg-white p-6 sm:p-7 rounded-3xl border border-slate-200/80 shadow-xs space-y-5">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div>
                <h3 className="text-base font-black text-slate-900">Diagnostik Server & Kesehatan Sistem</h3>
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold inline-flex items-center gap-2 cursor-pointer transition shadow-2xs disabled:opacity-50"
              >
                <RefreshCw size={13} className={loadingHealth ? 'animate-spin text-indigo-600' : ''} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 rounded-2xl bg-emerald-50/80 border border-emerald-200/80">
                ...
                <div className="text-lg font-black text-emerald-950 mt-2">Terhubung Normal</div>
              ...
              <div className="p-4 rounded-2xl bg-indigo-50/80 border border-indigo-200/80">
                ...
                <div className="text-lg font-black text-indigo-950 mt-2">Port 3000 Online</div>
                <div className="text-xs text-indigo-700 mt-1">Status: Full-Stack / Node.js tsx</div>
              </div>

              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                <div className="flex items-center gap-2 text-slate-700 font-bold text-xs">
                  <Cpu size={16} />
                  <span>Versi Platform Presensi</span>
                </div>
                <div className="text-lg font-black text-slate-900 mt-2">v2.5.0 Pro Multi-Tenant</div>
                <div className="text-xs text-slate-500 mt-1">KawaCanaan Architecture</div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                ...
                <div className="text-xs text-slate-500 mt-1">Zona Waktu: Asia/Jakarta</div>
              </div>
            </div>
          </div>

          {/* Kolom Kanan: Form Ubah Password Super Admin (5/12) */}
          <div className="lg:col-span-5 bg-white p-6 sm:p-7 rounded-3xl border border-slate-200/80 shadow-xs space-y-5">
            <div className="pb-3 border-b border-slate-100">
              <h3 className="text-base font-black text-slate-900">Kata Sandi Akun Super Admin</h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Perbarui kata sandi utama akun Super Administrator platform.
              </p>
            </div>
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-xl border border-slate-200 text-xs font-medium focus:outline-indigo-600"
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold shadow-xs flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                >
