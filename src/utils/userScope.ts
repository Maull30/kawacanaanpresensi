...
export function getUserRoleScope(
  ...
  if (isSuperAdmin || isAdmin || isKepalaSekolah) {
    ...
    // Ruang Kerja Individu & Wali Kelas: Kapasitas tepat 1 kelas (data & visualisasi terbatas untuk 1 kelas binaan)
    const effectiveWali = assignedWaliClass || (classes.length > 0 ? classes[0] : null);
    accessibleClasses = effectiveWali ? [effectiveWali] : [];
  } else if (isGuruMapel) {
